export const metadata = {
  ...
  async function getEvents(page = 1, limit = 12) {
    ...
    const res = await fetch(`${base}/events?page=${page}&limit=${limit}`, { next: { revalidate: 60 } });
    ...
    return (
      <main className="mx-auto max-w-6xl px-4 py-8">
        ...
        <div className="flex items-center gap-2 mt-8">
          {page > 1 && (
            <a href={`/events?page=${page - 1}`} className="px-3 py-1 border rounded">Previous</a>
          )}
          <span className="text-sm text-gray-600">Page {pagination.page} of {pagination.pages}</span>
          {pagination.page < pagination.pages && (
            <a href={`/events?page=${page + 1}`} className="px-3 py-1 border rounded">Next</a>
          )}
        </div>
      </main>
    );
